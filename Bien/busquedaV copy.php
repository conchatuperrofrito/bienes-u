...
if ($optiona1 != null) {
    ...
    for ($ia = 0; $ia < $conta; $ia++) {
        $where2 .= $columns1[$ia] . " LIKE '%" . $optiona1 . "%' OR ";
    }
...
if ($optionb1 != null) {
    ...
    for ($ib = 0; $ib < $contb; $ib++) {
        $where3 .= $columns1[$ib] . " LIKE '%" . $optionb1 . "%' OR ";
    }
    $where3 = substr_replace($where3, "", -3);
...
if ($optionc != null) {
    ...
    // 5 VECES 
    for ($ib = 0; $ib < $contb; $ib++) {
        $where4 .= $columns1[$ib] . " LIKE '%" . $optionc . "%' OR ";
    }
    $where4 = substr_replace($where4, "", -3);
...
FROM $table1
$where1 AND $where2 AND $where3 AND $where4
$sOrder1
$sLimit1";
