...
if (isset($_GET['delete'])) {
  ...
  $delete_product_image = $conn->prepare("SELECT * FROM `tinventario` WHERE inven_codigo= ?");
  $delete_product_image->execute([$delete_id]);
  $fetch_delete_image = $delete_product_image->fetch(PDO::FETCH_ASSOC);
  unlink('../Bien/conection/carpeta_pdf/' . $fetch_delete_image['pdfCot']);
  ...
  while ($CotDeleterow = $CotDeleteselect_products->fetch(PDO::FETCH_ASSOC)) {
    ...
    $CotDeleteinsert_control->execute(['0', 'ELIMINO UN BIEN,  NUMERO:' . $delete_id, $CotDeleterow['tadmin_nombre'], $fechaActual, $CotDeleterow['tadmin_dni'],]);
  }
  header('location:dashboard.php');
}
if (isset($_GET['deletee'])) {
  ...
  if ($fetch_deletet) {
    // Elimina el archivo
    unlink('../Bien/Admin/filePDFcotizacion/' . $fetch_deletet['tsuvirpdf_direccionPDF']);
...
<body>
  ...
  <header>
    ...
    <nav>
      <ul>
        <li>
          <a href="javascript:void(0);">AGREGAR UN BIEN</a>
        </li>
        <li class="<?= $row['tadmin_tipo'] ?>">
          <a href=" javascript:void(0);">TAREAS REGISTRDAS</a>
        </li>
        <li class="<?= $row['tadmin_tipo'] ?>">
          <a href="javascript:void(0);">BIENES</a>
        </li>
        <li>
  ...
  <main>
    <div class="title">
      <h5 id="fechaHoraActualizacion">
        <i class="ri-calendar-schedule-fill" style="color: #0498ff; "></i>
      </h5>
      <script>
      ...
      <a id="user-btn" style="
    font-size: x-large;
"><i class="ri-user-2-fill" style="color: #0498ff; "></i> HOLA ING :
    ...
    <div class="profile">
      ...
      ?>
      <p id="roltipo">
        <i class="ri-user-settings-line" style="color: #0498ff; "></i>

        <?= $fetch_profile['tadmin_tipo']; ?>
      </p>
      <p id="rolestado">
        <i class="ri-bubble-chart-line" style="color: #0498ff; "></i>
        <?= $fetch_profile['tadmin_estado']; ?>
      </p>
      <a href="profile/update_profile.php" class="option-btn roltipo">
        <i class="ri-folder-user-line" style="color: #0498ff; ">
        </i> actualizar perfil</a>
      <div id="admin" class="flex-btn <?= $row['tadmin_tipo'] ?> roltipo">
        <a href="profile/register_admin.php" class="option-btn "> <i class="ri-arrow-go-forward-line"
            style="color: #0498ff; "></i>registrar</a>
        <!-- <a href="admin_login.php" class="option-btn ">soy usuario</a> -->
      </div>
      <div id="admin" class="flex-btn <?= $row['tadmin_tipo'] ?> roltipo">
        <a href="../Bien/Admin/verUsuarios/usuarioIndex.php" class="option-btn "><i class="ri-folder-user-line"
            style="color: #0498ff; "></i> ver usuarios</a>
        <!-- <a href="admin_login.php" class="option-btn ">soy usuario</a> -->
      </div>

      <a href="profile/admin_logout.php" class="delete-btn roltipo"
        onclick="return confirm('¿cerrar sesión en el sitio web?');"><i class="ri-login-box-line"
          style="color: #0498ff; "></i> cerrar
        sesión
      </a>
    </div>
    <script>
    ...
    <article class="larg">
      <div>
        <h3 onclick="mostrarContenido()"><i class="ri-chat-settings-fill" style="    color: #0498ff;"></i> Tiene
          recomendaciones. <i class="ri-skip-down-line" style="    color: red;"></i></h3>
        <div class="contenido">
          <p class="contenido">
            <i class="ri-asterisk" style="    color: #0498ff;"></i> Asegúrate completar de manera corecta cada bien.
          </p>
          <p class="contenido">
            <i class="ri-asterisk" style="    color: #0498ff;"></i> Está supervisado por un control de actividades.
          </p>
          <p class="contenido">
            <i class="ri-asterisk" style="    color: #0498ff;"></i> Comunicate con soporte en caso de que tengan
            preguntas
            o necesiten ayuda para completar el registro de bienes.
          </p>
        </div>
      </div>

      <?php include '../Bien/Admin/admin.php'; ?>
    </article>
    <!-- ARTICULO 7 -->
    ...
    <article class="larg <?= $row['tadmin_tipo'] ?>">
      <!-- <div>
        <h3 onclick="mostrarContenido()"><i class="ri-chat-settings-fill" style="    color: #0498ff;"></i> Tome un
          momento para
          familiarizarse con las recomendaciones. <i class="ri-skip-down-line" style="    color: red;"></i></h3>
        <p class="contenido">
          <i class="ri-asterisk" style="    color: #0498ff;"></i> Los registros se eliminaran periodicamente
          automatiamente.
        </p>
      </div> -->

      <?php
      include '../Bien/Admin/verControl.php';
    ...
    <article class="larg">
      <!-- <div>
        ...
        <div class="contenido">
          <p class="contenido">
            <i class="ri-asterisk" style="    color: #0498ff;"></i> Asegúrate completar la cotización, sean claras y
            fáciles de entender.
          </p>
          <p class="contenido">
            <i class="ri-asterisk" style="    color: #0498ff;"></i> Está protegido contra alteraciones de scripts
      ...
      <!-- traba inicio   -->
      <?php
      require '../Bien/index.php';
      // system('php ../Bien/index.php');
      ?>
      <!-- traba fin   -->
    ...
    <article class="larg">
      <div>
        <h3 onclick="mostrarContenido()"><i class="ri-chat-settings-fill" style="    color: #0498ff;"></i>mensaje
          contat<span class="entypo-down-open"></span></h3>
        <p class="contenido">
        ...
        <div class="box-container mensage">
          ...
          if ($select_messages->rowCount() > 0) {
            while ($fetch_message = $select_messages->fetch(PDO::FETCH_ASSOC)) {
              ?>
              <h4><i class="ri-code-line" style="color: #0498ff;"></i>&nbsp;&nbsp;Código de mensaje
                :
                <span>
                  <?= $fetch_message['id']; ?>
                </span>
              </h4>
              <h4><i class="ri-anticlockwise-line" style="color: #0498ff;"></i>&nbsp;&nbsp;Enviado desde la cuenta de
                :<span>
                  <?= $fetch_message['user_id']; ?>
                </span></h4>
              <h4><i class="ri-message-2-line" style="color: #0498ff;"></i>&nbsp;&nbsp;mensaje
                :
                <span>
              ...
              <h4><i class="ri-user-3-line" style="color: #0498ff;"></i>&nbsp;&nbsp;Nombre emitente
                :
                <span>
                  <?= $fetch_message['name']; ?>
                </span>
              </h4>
              <h4><i class="ri-calendar-schedule-line" style="color: #0498ff;"></i>&nbsp;&nbsp;UTF hora y fecha
                :&nbsp
                <span>
