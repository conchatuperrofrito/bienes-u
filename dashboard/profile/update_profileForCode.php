...
if (isset($_POST['submit'])) {
   ...
   if ($old_pass == $empty_pass) {
      ...
      if ($new_pass != $empty_pass) {
         ...
         $update_admin_pass->execute([$confirm_pass, $update_dni]);
   ...
   while ($row = $select_products1->fetch(PDO::FETCH_ASSOC)) {
      ...
      $insert_control1->execute(['0', 'ACTUALIZO SU PERFIL DE:  ' . $update_dni . ' ->', $row['tadmin_nombre'], $fechaActual, $row['tadmin_dni'],]);
   }
...
<body>
   <section class="form-container">
      ...
      $select_profile->execute([$update_dni]);
